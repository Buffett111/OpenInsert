...
0.2.8 的綠色聲波使用既有每 100 ms PCM RMS 音量，僅在 HUD 以 −55 至 −12 dB 映射為條柱高度，不調整或額外傳送音訊。保留最近 31 個音量樣本，靜音也推進歷史；這是音量包絡而非頻譜。聲波獨占 28 px 高度，轉錄文字放在下方；錄音中的快捷鍵／技術說明不再佔用字幕區，錯誤與複製提示保留。減少動態效果設定會停用插值動畫；僅明確標示的離線預覽使用合成波動。
