...
0.2.7 在貼上事件派送後透過 nonthrowing callback 同步收起 HUD；該次操作仍在背景等候 800 ms 剪貼簿恢復，期間不重顯面板，結束時也不再顯示兩秒完成訊息。派送旗標受操作 generation 約束，不能作為目的地已接收文字的證據。自動複製通知及錯誤仍分別顯示約六秒及十秒。

0.2.8 的綠色聲波使用既有每 100 ms PCM RMS 音量，僅在 HUD 以 −55 至 −12 dB 映射為條柱高度，不調整或額外傳送音訊。保留最多 96 個音量樣本，以 2 pt 細柱及 2 pt 間距呈現，窄畫面顯示較少的最近樣本；靜音也推進歷史。這是音量包絡而非頻譜。面板首選寬度為 400 pt，最多使用目前螢幕可用寬度減 32 pt；聲波獨占 28 px 高度，轉錄文字放在下方。錄音中的快捷鍵／技術說明不再佔用字幕區，錯誤與複製提示保留。減少動態效果設定會停用插值動畫；僅明確標示的離線預覽使用合成波動。
