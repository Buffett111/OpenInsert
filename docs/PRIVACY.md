...
This describes OpenInsert 0.2.6, currently under validation and not released. OpenInsert has no product account, developer-operated server, analytics SDK, screenshot capture, OCR, or transcript-history database.
...
**Automatic copy after completed dictation:** If final text is ready but no valid input target was captured, OpenInsert copies the result to the clipboard instead of pasting into an unverified app. This also applies to supported insertion errors before paste dispatch. It does not retry a copy after empty-text, clipboard-change, clipboard-read or clipboard-write errors. Cancellation, unfinalized output, unhandled provider failures, unknown errors and errors after paste dispatch do not trigger automatic copy. An accepted skip or temporary-cleanup fallback still uses finalized ASR and can reach this delivery path. The copied status is shown only after a successful clipboard write; a copy failure retains the result in the app and reports an error.

Automatic copy intentionally replaces the current clipboard and leaves the result there, even when paste restoration is enabled. It does not back up or restore previous clipboard content. You can paste the result without reopening OpenInsert; clearing the app result or quitting does not clear the clipboard. The floating copied-status display lasts about six seconds without activating the main window. The five-second fixed-text insertion test uses the same delivery rules without microphone capture or an API request. Clipboard managers and system clipboard features may retain or sync this result.

**Other apps and clipboard:** Accessibility is used to validate the focused app, element role/protection state and selection range. Focus is queried globally first; only specific missing/unsupported results allow one query against the expected app. The foreground and returned-element PIDs must match that app. No parent/child UI search is performed. A writable selected-text capability may be checked as evidence of editability, but selected text is not read and transcript text is not written through AX attributes. OpenInsert does not read the text around your cursor or send accessibility metadata to Google. Every automatic insertion temporarily writes the result to the system clipboard and requests standard Command-V. With clipboard restoration enabled, existing clipboard formats are read into memory solely for restoration and are never uploaded. After 800 ms, restoration occurs only if OpenInsert still owns the same clipboard version. A paste request has no OS delivery receipt; the app does not retry with an AX text write. With restoration disabled, the result stays on the clipboard. Copying a result manually also intentionally leaves it there. The destination app, clipboard managers and system clipboard features may retain or sync inserted/copied text under their own settings.

With Accessibility already granted, OpenInsert can prepare the foreground app's accessibility bridge by reading bundle metadata, the application role, and the actual `AXManualAccessibility` capability and Boolean flag. It no longer requires a particular framework filename. A writable, disabled Manual flag can be enabled. Only the exact bundle principal class `BrowserCrApplication` allows the alternative `AXEnhancedUserInterface` setter when Manual is unavailable; Manual permission/transport errors or invalid Boolean data do not permit that fallback. This targets a Chromium implementation, not an app's display name, and does not bypass system permission.

During each OpenInsert run, an activation write is attempted at most once for each identified target process launch, including failed writes. Flags are not disabled or repeatedly rewritten. Only a successful setter starts a three-second preparation window; valid focus is still required and other focus/selection failures retain their AX attribute and error code. The latest preparation diagnostic stays in memory and contains only the app bundle ID, principal class, AX status, Boolean flag and attempted action. These checks do not enumerate UI children, read input or surrounding text, or upload app/AX metadata. See [the implementation and Chromium source evidence](ARCHITECTURE.md#捕捉與再驗證).
...
**完成後自動複製：** 最終文字準備好但未捕捉到有效輸入目標時，OpenInsert 會把結果複製到剪貼簿，不往未驗證的 App 貼上；貼上派送前遇到可回退的插入錯誤也適用。空文字、剪貼簿已變更、無法備份或寫入失敗時不再嘗試複製；取消、未定稿、未被處理的供應商失敗、未知錯誤或貼上已派送後的錯誤亦不自動複製。原本已接受的略過或暫時後修失敗，若採用定稿 ASR，仍可走此送達流程。只有寫入成功才顯示已複製；失敗顯示錯誤，結果留在 App 供手動取用。

自動複製會刻意取代目前剪貼簿並持續保留結果，即使啟用「恢復剪貼簿」也不備份或恢復原資料。你可以直接貼上，不必開啟 OpenInsert；清除 App 結果或結束 App 不會清空剪貼簿。HUD 約顯示 6 秒已複製狀態、不啟用主視窗。「測試文字插入（5 秒倒數）」也採相同送達規則，不錄音、不使用 API。剪貼簿管理器與系統剪貼簿功能可能保存或同步這份結果。

**其他 App 與剪貼簿：** 輔助使用 API 檢查目前 App、焦點元件角色／保護狀態及選取範圍；先查全域焦點，只有特定缺值／不支援結果才改查預期 App 一次，前景與回傳元件的 PID 都必須符合，不搜尋 parent／children；可能查詢選取文字屬性是否可寫作為可編輯證據，但不讀選取文字、不透過 AX 屬性寫入逐字稿，也不讀游標周圍文字或將定位資訊送 Google。每次自動插入都暫用系統剪貼簿並請求標準 Command-V。啟用恢復時，原剪貼簿資料只在記憶體內備份、不上傳，等待 800 ms 後僅在仍持有同一剪貼簿版本時恢復。貼上沒有系統收件證明，不會再以 AX 寫入重試；停用恢復或手動複製會把結果留在剪貼簿。目的 App、剪貼簿管理器及系統剪貼簿功能可能依各自設定保存或同步文字。

已有輔助使用授權時，OpenInsert 可讀取前景 App 的 bundle metadata、application role，以及實際 `AXManualAccessibility` 可寫能力與布林旗標，協助建立原生 AX 介面，不再要求特定 framework 檔名。Manual 可寫且尚未啟用時才設定；只有 bundle principal class 完全等於 `BrowserCrApplication`，且 Manual 無法使用時，才允許改要求 `AXEnhancedUserInterface`。Manual 查詢的權限／通訊錯誤或無效布林值不允許這條替代路徑。此條件針對 Chromium 實作，不依 App 顯示名稱猜測，也不繞過系統權限。

在每次 OpenInsert 執行期間，每個可識別的目標程序啟動最多嘗試一次初始化寫入，即使失敗也不重送，不停用旗標。只有設定成功才保留 3 秒準備期間；仍須取得有效焦點，其他焦點／選取範圍失敗保留 AX 屬性與錯誤代碼。最近一次初始化診斷只在記憶體保留 App bundle ID、principal class、AX status、布林旗標及嘗試動作。這不列舉 UI 子元件、不讀輸入內容或周圍文字，也不上傳 App／AX 定位資訊。實作與 Chromium 來源依據見 [架構文件](ARCHITECTURE.md#捕捉與再驗證)。
