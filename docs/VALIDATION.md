...
記錄日期：2026-09-19；目前原始碼驗證版本：0.2.8／build 10；公開 prerelease 為 0.2.7。測試環境為 macOS 27.0（26A428）、arm64、Apple Swift 6.4、Command Line Tools MacOSX27.0 SDK。此頁區分新版已執行檢查、待驗證項目與歷史證據；建置成功不代表真實語音辨識及所有應用程式插入已驗證。

## 0.2.8 綠色音量聲波（未發布）

- **實作**：將灰色線性音量條換成真實 PCM RMS 驅動的綠色條柱，以 100 ms 既有取樣推進最近 31 筆音量歷史；靜音推進為最小高度，不加入假波動。顯示端 dB 映射不變更 ASR 音訊。錄音時移除常駐快捷鍵說明，聲波獨占 28 px，轉錄文字置於下方；0.2.7 的貼上後立即收起、自動複製與錯誤提示行為保留。
- **編譯及簽章通過**：macOS 13／Swift 5 完整 App typecheck、arm64／x86_64 universal 建置及 strict codesign 通過；新 staging bundle 為 0.2.8／build 10，designated requirement 與已安裝的 0.2.7 相同。未修改音訊擷取、轉換、快捷鍵、ASR 或文字插入程式碼。
- **離線視覺檢查通過**：直接使用本次 SwiftUI 模型與 View，生成 480／320 × 176、淺色／深色、錄音長字幕／自動複製／錯誤共 12 張快照。聲波與字幕無重疊，複製及錯誤訊息無非預期裁切；320 寬的長字幕按四行限制省略部分內容。此檢查使用合成音量與本機 NSHostingView，不開麥克風、不連線 Google，也不代表真實收音或材質透視效果已驗證。
- **審閱確認**：新錄音清除舊音量、相同音量及靜音仍推進歷史，文字更新不新增音量取樣；Reduce Motion 關閉插值並暫停模擬動畫。模擬波動僅出現在有標示的八秒離線預覽。真實麥克風音量與新版 HUD 的互動待安裝後確認。
- **安裝與原生 UI 確認**：已更新 `/Applications/OpenInsert.app` 至 0.2.8／build 10，strict codesign 通過，0.2.7 已備份。UI 顯示 API key 已儲存、Google 同意保留、麥克風與輔助使用已啟用，Option-Space、繁體中文偏好及既有輸出模式保留。未重設權限、未讀出金鑰；已點選新版八秒聲波預覽。UI 工具僅擷取主視窗，浮動 panel 外觀以離線快照驗證；未宣稱本次已實測真實麥克風動畫。
